#!/usr/bin/env python3
"""
Pipeline — Conjuntos numericos N Z Q R (ConjuntosNumericos)

Pasos:
  0. Calcular section_starts desde audios (ffprobe)
  1. Render Manim (1080p60)
  2. Integrar 7 audios con ffmpeg
...
MANIM_SCRIPT = ROOT / "video_conjuntos_numericos.py"
SCENE = "ConjuntosNumericos"
MANIM_OUTPUT = (
    ROOT
    / "media"
    / "videos"
    / "video_conjuntos_numericos"
    / "1080p60"
    / f"{SCENE}.mp4"
)
FINAL_OUTPUT = ROOT / "video_conjuntos_numericos_final.mp4"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pipeline ConjuntosNumericos")
    parser.add_argument("--skip-sync", action="store_true")
    parser.add_argument(
        "--skip-manim",
        action="store_true",
        help="Omitir render si ya existe ConjuntosNumericos.mp4",
    )
    args = parser.parse_args()

    print("=" * 60)
    print("Pipeline — Conjuntos numericos")
    print("=" * 60)
    print()
