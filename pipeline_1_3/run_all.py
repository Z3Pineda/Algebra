#!/usr/bin/env python3
"""
Pipeline completo Video 1.3 — Comprension

Pasos:
  0. Calcular section_starts.json desde audios/ (ffprobe)
  1. Render Manim (Comprension, 1080p60) — usa section_starts.json
  2. Integrar MP3 con ffmpeg → video_1_3_final.mp4

Uso:
  python run_all.py
  python run_all.py --skip-manim
  python run_all.py --skip-sync     # si ya existe section_starts.json
"""
...
MANIM_SCRIPT = ROOT / "video_1_3_comprension.py"
SCENE = "Comprension"
MANIM_OUTPUT = (
    ROOT / "media" / "videos" / "video_1_3_comprension" / "1080p60" / f"{SCENE}.mp4"
)
FINAL_OUTPUT = ROOT / "video_1_3_final.mp4"
...
def resolver_manim() -> str:
    ...
    print()
    print("O globalmente: pip install manim")
    sys.exit(1)
...
def paso_manim() -> Path:
    ...
    if not SYNC_FILE.exists():
        print("AVISO: No hay section_starts.json; Manim usara tiempos por defecto.")
        print("       Ejecuta primero sync_timeline.py o quita --skip-sync.")
        print()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pipeline Video 1.3 — Comprension")
    parser.add_argument(
        "--skip-sync",
        action="store_true",
        help="Omitir calculo de section_starts.json",
    )
    parser.add_argument(
        "--skip-manim",
        action="store_true",
        help="Omitir render Manim",
    )
    args = parser.parse_args()

    print("=" * 60)
    print("Pipeline Video 1.3 — Comprension")
    print("=" * 60)
    print()
