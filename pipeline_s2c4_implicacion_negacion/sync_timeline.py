#!/usr/bin/env python3
"""Linea de tiempo — ImplicacionNegacion."""
...
AUDIO_FILES = [f"s2c4_bloque{i}.mp3" for i in range(1, 11)]

SECTION_TITLES = [
    "Apertura horno",
    "Negacion",
    "Negacion en ingenieria",
    "De Morgan",
    "Implicacion",
    "p falsa en implicacion",
    "Horno industrial",
    "Negacion implicacion",
    "Tabla cuatro conectivos",
    "Cierre",
]
...
def calcular_linea_de_tiempo(ffprobe: str) -> list[BloqueTimeline]:
    ...
    for i, nombre in enumerate(AUDIO_FILES):
        ...
        bloques.append(BloqueTimeline(nombre, i, SECTION_TITLES[i], inicio, duracion))
        inicio += duracion
    return bloques
...
def main() -> None:
    ...
    print(f"section_starts exportado: {exportar_section_starts(bloques)}")
