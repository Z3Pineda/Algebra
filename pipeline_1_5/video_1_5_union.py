"""
Video 1.5 — Cómo calcular la unión de dos conjuntos
...
Renderizar (prueba):
    manim -pql video_1_5_union.py Union

Renderizar (final):
    manim -pqh video_1_5_union.py Union
...
def cargar_section_starts() -> list[float]:
    ...
    return [0.0, 12.199, 25.992, 72.673, 94.302]
...
class Union(Scene):
    ...
    def _escena_definicion(self):
        ...
        venn = VGroup(circ_a, circ_b, lbl_a, lbl_b).move_to(ORIGIN + DOWN * 0.8)

        nota = Text(
            "La región sombreada es A ∪ B",
            font_size=22, color=GRIS, slant=ITALIC,
        ).next_to(venn, DOWN, buff=0.4)
    ...
    def _escena_procedimiento(self):
        ...
        paso1_lbl = Text("Paso 1 — Listar todos los elementos de A y B",
        ...
        paso2_lbl = Text("Paso 2 — Eliminar los repetidos",
                         font_size=22, color=AMARILLO, weight=BOLD)
        paso2_ej  = MathTex(
            r"6,\ 8,\ 10,\ 12,\ 16",
            font_size=32, color=WHITE,
        ...
        paso3_lbl = Text("Paso 3 — Escribir el resultado entre llaves",
                         font_size=22, color=AMARILLO, weight=BOLD)
        paso3_ej  = MathTex(
            r"A \cup B = \{6,\ 8,\ 10,\ 12,\ 16\}\ \text{mm}",
        ...
        pasos.next_to(conjuntos, DOWN, buff=0.45)
    ...
    def _escena_errores(self):
        ...
        errores = [
            (
                r"A \cup B = \{6,\ 8,\ 10,\ 12,\ 8,\ 12,\ 16\}",
                r"A \cup B = \{6,\ 8,\ 10,\ 12,\ 16\}",
                "Repetir elementos en el resultado",
            ),
            (
                r"A \cup B = \{8,\ 12\}",
                r"A \cup B = \{6,\ 8,\ 10,\ 12,\ 16\}",
            ...
            (
                r"A \cup B = \{6,\ 10\}",
                r"A \cup B = \{6,\ 8,\ 10,\ 12,\ 16\}",
                "Incluir solo los elementos exclusivos de A",
            ),
        ]
        ...
        for mal, bien, razon in errores:
            f_mal  = MathTex(mal,  font_size=24, color=ROJO)
            flecha = Text("→", font_size=22, color=GRIS)
            f_bien = MathTex(bien, font_size=24, color=VERDE)
    ...
    def _escena_cierre(self):
        definicion = Text(
            "A ∪ B contiene todo lo que está en A,\n"
            "en B, o en ambos. Sin repetir elementos.",
            font_size=32, color=WHITE, weight=BOLD, line_spacing=1.4,
        ).move_to(ORIGIN + UP * 0.4)

        siguiente = Text(
            "Siguiente: 1.6 · Intersección de conjuntos  →",
...
if __name__ == "__main__":
    print("Renderiza con: manim -pql video_1_5_union.py Union")
