#!/usr/bin/env python3
"""Integra TresFormas.mp4 (Manim, sin audio) con 6 bloques MP3 usando ffmpeg."""
...
OUTPUT = ROOT / "video_tres_formas_final.mp4"

VIDEO_CANDIDATES = [
    ROOT / "TresFormas.mp4",
    ROOT / "media" / "videos" / "video_tres_formas" / "1080p60" / "TresFormas.mp4",
]
...
def verificar_ffmpeg() -> tuple[str, str]:
    ...
    if not ffmpeg or not ffprobe:
        ...
        print("  - Reinicia la terminal despues de instalar.")
        sys.exit(1)
...
def buscar_video() -> Path:
    for candidato in VIDEO_CANDIDATES:
        if candidato.exists():
            return candidato
    print("ERROR: No se encontro TresFormas.mp4")
    print("Renderiza primero:")
    print("  manim -pqh video_tres_formas.py TresFormas")
    sys.exit(1)
...
def main() -> None:
    print("=" * 50)
    print("Integracion — TresFormas + 6 audios")
    print("=" * 50)
    print()
